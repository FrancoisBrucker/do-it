...
- Pour cartographier les flux on utilise un VSM (Value Stream Mapping). Cela permet d'identifier le temps passer sur des activités sans valeur ajoutées
...
- Les équipes du train sont synchronisée : Elles ont toutes le même temps de PI et ont les mêmes dates de début et fin des itérations. Le planning du train est établi périodiquement lors de rencontres en face à face avec les équipes.
...
Cette méthode fonctionne en cycles de 6 semaines suivits par des cool down de 2 semaines (objectif = éviter l'effet tunnel).
...
Lors du premier cycle, on va collecter les besoins des utilisateurs, cadrer la solution produit evisagée. L'idée est de “façonner”/”défricher” une idée brute. On doit êtablir clairement le problème auquel on veut répondre et les limites de notre solutions , décrire ses grandes fonctionnalités et les risques.
