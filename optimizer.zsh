...
if [[ "$1" == "image" || "$2" == "image" || "$3" == "image" ]]; then
    ...
    find ./src -type f \( -name "*.jpg" -o -name "*.jpeg" -o -name "*.png" \) | while read -r img; do
        img_clean="${img#./}" # Remove './' prefix
        ffmpeg -y -i "$img" -q:v 75 "${img_clean%.*}.webp" && \
    ...
    find ./src -type f \( -name "*.mp4" -o -name "*.mov" -o -name "*.mkv" \) | while read -r video; do
    ...
    find ./src -type f \( -name "*.wav" -o -name "*.flac" -o -name "*.mp3" -o -name "*.ogg" \) | while read -r audio; do
        audio_clean="${audio#./}" # Remove './' prefix
        ffmpeg -y -i "$audio" -c:a libopus -b:a 96k "${audio_clean%.*}.opus" && \
